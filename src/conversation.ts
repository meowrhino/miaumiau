...
// EL MOTOR de chat en vivo, trasplantado de rumrum (allí: sala anónima; aquí:
// una sala por zona del parque). Es agnóstico al producto: solo sabe de
// mensajes, orden, perfiles (color) y fan-out en tiempo real. La identidad
// vive FUERA (el Worker valida el token y le pasa el nombre ya verificado).
...
// Color por defecto determinista para quien llegue sin color en el query.
...
/**
 * ConversationDO — una instancia por sala (= zona del parque, o 'plaza').
 *
...
export class ConversationDO extends DurableObject<Env> {
  ...
  // El Worker enruta aquí el upgrade de WebSocket con `name`/`color` YA
  // autenticados en el query (ver la ruta /ws en src/index.ts).
  ...
  async webSocketMessage(ws: WebSocket, raw: string | ArrayBuffer): Promise<void> {
    ...
    if (data.type === "color") {
      ...
      if (!color) return;
      // No-op: si ya tiene ese color, ni escribimos ni difundimos.
  ...
  // para no pisar el color elegido con el del query-param. El color solo
